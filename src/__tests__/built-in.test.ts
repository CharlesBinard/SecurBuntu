...
describe("BUILT_IN_PRESETS", () => {
  ...
  test("minimal has UFW with SSH only", () => {
    const m = BUILT_IN_PRESETS.minimal as Preset
  ...
  test("web-server has HTTP/HTTPS ports and Fail2ban", () => {
    const ws = BUILT_IN_PRESETS["web-server"] as Preset
  ...
  test("database has kernel hardening and file permissions", () => {
    const db = BUILT_IN_PRESETS.database as Preset
  ...
  test("fortress has everything enabled", () => {
    const f = BUILT_IN_PRESETS.fortress as Preset
